...
public class Pusher {
	...
	public Pusher(String host, int port, int timeoutMillis, int version,
	...
	private boolean checkUuidArray(byte[] uuid) throws IllegalArgumentException {
		if (uuid == null || uuid.length != 8) {
			throw new IllegalArgumentException(
					"uuid byte array must be not null and length of 8");
		}
		return true;
	}

	private boolean checkLongArray(byte[] longArray)
			throws IllegalArgumentException {
		if (longArray == null || longArray.length != 8) {
			throw new IllegalArgumentException(
					"array must be not null and length of 8");
		}
		return true;
	}
	...
	public boolean push0x20Message(byte[] uuid, byte[] data) throws Exception {
		...
		if (data.length == 0 || data.length > 500) {
			throw new IllegalArgumentException(
					"data array length illegal, min 1, max 500");
		}
	...
	public boolean push0x20MessageCheck(byte[] uuid, byte[] data)
			throws Exception {
		this.checkUuidArray(uuid);
		if (data == null) {
			throw new NullPointerException("data array is null");
